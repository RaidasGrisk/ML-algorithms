...
# List of variables:
# L_Empty   -   An empty NN shell of chosen size. All neurons are equal to zeros. Created using function LayerPrep().
# L         -   A filled NN shell of chosen size. Created using function ForwardProp().
# O         -   A dictionary where weights are stored. Created using function WightsPrep().
# O_grad    -   A dictionary where gradients of weights are stored. Created using function G().
# A         -   A dictionary of z values used in backpropagation. Created using function ForwardProp().
...
def checkgradients(O, alpha, show, biggestdiff):
    ...
    for i in range(0, len(O)):
        ...
        print(i, 'of', len(O))
    fungrad = np.reshape(G(O), (len(G(O)), 1))
...
def interm(O_interm):
    print('Train error:', np.around(J(O_interm, *args), 2),
          'Precision:',accuracy(y, ForwardPropagation(O_interm, L_empty)[2], 0.5)[0], '  |  ',
          'Test error:', np.around(J(O_interm, *args_cv), 2),
          'Precision:', accuracy(y_cv, ForwardPropagation(O_interm, L_empty_cv)[2], 0.5)[0])
...
# Data
# Datapoints - rows; features - columns
y_full = pd.read_csv('y_data.txt', delimiter=',', header = None).values
x_full = pd.read_csv('x_data.txt', delimiter=' ', header = None).values
...
O, L_empty, L, A, y_est, args, L_empty_cv, L_cv, A_cv, y_est_cv, args_cv, l = initializeNN(
                                                                                        ...
                                                                                        x = x_full[0:9000,:],
                                                                                        y = y_full[0:9000,:],
                                                                                        x_cv = x_full[9000:10000,:],
                                                                                        y_cv = y_full[9000:10000,:])

# Optimization
O_result = optimize.fmin_cg(J, O, fprime=G, args=args, maxiter=100, disp=True, callback=interm, retall=True)
plotresults(O_result)

# Save opt O's to continue optimization further
O = O_result[0]

# Print Precision with different threshold
for i in range(1, 10):
        threshold = 0.25 + i/50
        print(threshold, accuracy(y_cv, ForwardPropagation(O_result[0], L_empty_cv)[2], threshold))

#Checking gradients
checkgradients(O, alpha=0.0001, show=1, biggestdiff=0)
