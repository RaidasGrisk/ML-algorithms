...
# initialize model
tf_model = {}
tf_model['W1'] = tf.Variable(tf.truncated_normal(shape=[n_obs, h], mean=0, stddev=1. / np.sqrt(n_obs), dtype=tf.float32, name='W1'))
tf_model['W2'] = tf.Variable(tf.truncated_normal(shape=[h, n_actions], mean=0, stddev=1. / np.sqrt(h), dtype=tf.float32, name='W2'))

Trained_weighs = np.load('Pong_v2_trained_weights.npy').item()
tf_model['W1'] = tf.Variable(Trained_weighs['W1'])
tf_model['W2'] = tf.Variable(Trained_weighs['W2'])
...
np.save('Pong_v2_trained_weights', sess.run(tf_model))
